...
pub async fn generate_with_sdcpp(
    ...
    if let Some(reference_asset) = reference_asset {
        let reference_path = reference_asset.disk_path(input_dir, outputs_dir);
    ...
    if let Some(end_reference_asset) = end_reference_asset {
        let end_reference_path = end_reference_asset.disk_path(input_dir, outputs_dir);
...
async fn prepare_control_video_frames(
    ...
    let extraction = async {
        match lower.as_str() {
            ...
            _ => {
                bail!(
                    "Control video '{}' is not a supported video source yet. Use a GIF, MP4, AVI, WEBM, MOV, or MKV file from the tray.",
...
async fn extract_video_frames_with_ffmpeg(video_path: &Path, output_dir: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Chatty-art could not unpack '{}' into control-video frames. Install ffmpeg or use a GIF from the tray.\n{}\n{}",
            video_path.display(),
            summarize_output("stdout", &stdout),
            summarize_output("stderr", &stderr)
        );
